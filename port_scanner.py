...
Python Port Scanner
--------------------
A multithreaded TCP port scanner that identifies open ports and attempts
to detect the service running on each port (via known port mapping and
banner grabbing).

⚠️ Ethical use only: only scan hosts you own or have explicit permission
to test. Unauthorized port scanning may violate laws such as the IT Act
(India) or the Computer Fraud and Abuse Act (US).

Usage:
    python port_scanner.py -t 127.0.0.1 -p 1-1024
    python port_scanner.py -t example.com -p 20,21,22,80,443
"""
...
# Common ports mapped to their typical service names
COMMON_PORTS = {
...
def main():
    ...
    scan(target_ip, ports, args.threads, args.timeout)
